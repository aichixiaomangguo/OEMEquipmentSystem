...
public class AddEquipActivity extends Activity implements View.OnClickListener {
    ...
    public void initView() {
        ...
        if(BaseUtil.getModel().startsWith("un")){
            txt_add_model.setText("HTC A9W");
        }
        else
        ...
        txt_add_memory.setText(BaseUtil.getMemory() + " GB");
        txt_add_storage.setText(BaseUtil.getTotalInternalStorageSize()+" GB");

        if(BaseUtil.getCPU().startsWith("0"))
            txt_add_cpu.setText("AArch64 Processor rev 0");
        else
